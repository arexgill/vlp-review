...
- Revision: 224d2f5cbf2b4bc2ebb7482d0592149a8d9f0574
- Clone method: `git clone --depth 1`
- Source root: `src/` staged at `/tmp/tavlin-vlp-hono-input/src`
- Included supported files: 186
- Largest included file: `src/types.ts` (89829 bytes)
...
All non-test supported source under Hono `src/` is analyzed through a disposable input copy, while tests and source outside `src/` are excluded. This exercises core route matching, middleware composition, and request/response code but is not whole-repository coverage.
